...
// inventing a fourth ambiguous hue: gold is reserved for the genuine
...
function tierColor(score: number): { stroke: string; text: string } {
  if (score >= 90) return { stroke: "stroke-gold", text: "text-gold" };
...
export function HealthScoreGauge({
  ...
  return (
    <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90" role="img" aria-label={`Health score: ${result.score} out of 100, ${result.label}`}>
        <circle
          cx={size / 2}
          cy={size / 2}
          r={radius}
