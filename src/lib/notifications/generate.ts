...
// Retention pass: "Your recurring spending increased by $X/month" — the
// brief's own flagship "money found" example, and the one thing this app
// could compute all along (a portfolio total) but never actually told
// anyone had changed. Deliberately NOT generated by generateNotificationCandidates
// above (called on every page load): a meaningful "since last time" figure
// only exists relative to a real anchor point, and the weekly digest is the
// one place this app already keeps one (users.lastDigestMonthlyCents) — the
// same reason buildUnusualChargeCandidate/buildConnectionIssueCandidate
// above are called directly from their one owning job rather than from the
// generic per-page-load path.
//
// A decrease is deliberately never surfaced this way — "your spending went
// down" is good news, not a watchdog interrupt, and this app already has a
// dedicated, honest place for celebrating a real decrease (realized-savings
// on /savings, computed from actually-canceled subscriptions, never a
// portfolio-total dip that could just as easily be one subscription's
// currency reclassifying or a temporary gap in what's tracked).
const MIN_NOTIFIABLE_SPEND_INCREASE_CENTS = 50; // $0.50 — enough to filter
// out currency-rounding noise (monthlyCents itself rounds yearly/quarterly/
// weekly cycles to the nearest cent) without hiding a real, if small, creep.

export function buildSpendIncreasedCandidate(params: {
  previousCents: number;
  previousCurrency: string;
  currentCents: number;
  currentCurrency: string;
}): NotificationCandidate | null {
  // Currency mismatch means the portfolio's own primary currency shifted
  // between digests (a big new-currency subscription added, or the old
  // primary-currency ones all canceled) — not a comparable "went up," so
  // this is honestly skipped rather than compared across currencies.
  if (params.previousCurrency !== params.currentCurrency) return null;
  const deltaCents = params.currentCents - params.previousCents;
  if (deltaCents < MIN_NOTIFIABLE_SPEND_INCREASE_CENTS) return null;
  return {
    type: "spend_increased",
    title: `Your subscriptions cost ${formatCents(deltaCents, params.currentCurrency)} more per month than last time`,
    body: `Recurring spend went from ${formatCents(params.previousCents, params.currentCurrency)}/mo to ${formatCents(params.currentCents, params.currentCurrency)}/mo.`,
    severity: "warning",
    impactCents: deltaCents,
    currency: params.currentCurrency,
    subscriptionId: null,
    actionHref: "/dashboard",
    // Tied to the exact new total, not a date or a running counter: the
    // same total reported again next week (nothing changed further) is a
    // harmless no-op via onConflictDoNothing, same posture every other
    // candidate in this file follows; a genuinely different total next time
    // is a new dedupeKey and a new notification.
    dedupeKey: `spend_increased:${params.currentCents}:${params.currentCurrency}`,
  };
}
